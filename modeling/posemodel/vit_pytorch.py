...
This code is copy from https://github.com/heshuting555/TransReID.git
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm): #nn.GELU
        super().__init__()
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.cam_num > 0 and self.view_num > 0:
            if self.view_num == 1:
                x = x + self.pos_embed + self.sie_xishu * self.sie_embed[camera_id * self.view_num]
            else:
