...
'An Image Is Worth 16 x 16 Words: Transformers for Image Recognition at Scale' - https://arxiv.org/abs/2.11929
...
class g_ViT(nn.Module):
    ...
    def local_uncertainty_modeling(self, x, t_member, c_member, layout):
        ...
        for dis in result:
            ...
            # if(torch.pairwise_distance(dis[0], dis[1]).item() > sum):
            #     sum =  torch.pairwise_distance(dis[0], dis[1]).item()
...
class GVit(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, embed_dim=768,
        ...
        #一阶vit 提取单个人的成员外观特征
        self.g_vit = g_ViT(img_size=img_size, sie_xishu=sie_xishu, stride_size=stride_size, depth=4,
                           num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                           drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate)
